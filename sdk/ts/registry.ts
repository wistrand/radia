 ...
 * only `queryAll` and `readExhaustively` produce it.
 *
 * WHAT IT DOES NOT MEAN IS COMPLETE. `readExhaustively` brands its accumulation while reporting
...
/** One page of an exhaustive walk, built by `readExhaustively` and passed straight through by the caller.
...
export async function readExhaustively(
  ...
  return { records: unsafeAsPopulation(all, "readExhaustively paged to exhaustion or reported it could not"), complete, scanned: all.length };
}
