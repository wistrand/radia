...
Deno.test("[compartment] a RETIRED grant stops making a principal a crosser", async () => {
  await withSpace(async (c) => {
    ...
    const rows = await c.queryNewest<object>({ kind: "grant", match: { principal: analyst, kind: "public_summary" } }, 10);
    assertEquals(rows.length, 1);
    await c.put({ kind: "grant", body: { ...(rows[0].body), retired: true } }, `grant-retire:${rows[0].id}`);
