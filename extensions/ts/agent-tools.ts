 ...
 * OLDEST rows back, and reported one of them as the maximum: the failure this tool exists to
 * prevent, produced by the tool. The wire now REFUSES the near-miss, but that refusal cannot reach
 * this path, since the pattern is rebuilt here and the model's key never crosses the socket.
 */
function pickOrderBy(a: Record<string, unknown>): { path: string; dir?: "desc" }[] | undefined {
  return normalizeOrderBy(a.orderBy ?? a.order_by);
}
  ...
  return {
    ...
    request_grant: async (a, ctx) => {
      ...
      while (Date.now() < deadline) {
        ...
        const rows = await client.queryNewest<Record<string, unknown>>({ kind: "grant_request", match: { conversationId: ctx?.conversationId } }, 50);
        const decided = rows.map((r) => r.body).find((b) => b.decision && mine(b));
        if (!decided) continue;
    ...
    space_query: async (a) => {
      ...
      const page = await client.queryPage(
        { kind: String(a.kind ?? ""), match: a.match as Record<string, unknown> | undefined, orderBy: pickOrderBy(a) as never },
        limit + 1,
        undefined,
        { explain: true },
      );
    ...
    space_flows: async (a) => {
      const sum = Array.isArray(a.sum) ? a.sum.map(String).slice(0, 4) : undefined;
...
export const INSPECT_SCHEMAS: ToolDef[] = [
  ...
  { type: "function", function: { name: "space_query", description: "Find records by kind, with an optional match (equality/$gt/$in/$exists/…) and orderBy. orderBy is an array of {path, dir?} over the kind's SORTABLE paths only (list a kind's sortable paths with space_kinds), and only over fields in the record BODY. When a record was created is not a body field, so there is no way to sort by time. Without orderBy, records come back in ascending record-id order; that is stable, not arbitrary, but it means a `limit` gives you the OLDEST matches, never the newest. So this is the wrong tool for 'the most recent X': use space_events (the event log is in time order) or narrow the match instead. Returns up to `limit` (default 10, max 25) records with size-capped bodies, plus `more`: true when further records match. The result is then a PAGE, so never count or compute percentages from it (space_stats has per-kind totals). To answer 'which X had the most Y', orderBy the numeric path descending — and ALWAYS pair a desc sort with match {thatPath: {$exists: true}}: records MISSING the field sort FIRST on desc, so without the match the top of the page is the records that have no value at all. If no kind obviously carries the field, do not guess a path: space_kinds lists every kind's declared paths, and the kind that DECLARES the field is the one to query, not the kind the question happens to name. The conversation itself is records: kind 'message' with match {conversationId}, orderBy [{path:\"index\"}]. A `scope` in the answer means your GRANT narrowed the read: what came back is a slice of that kind, not the kind. Say so instead of describing it as everything, and do not try to widen it by guessing at kinds.", parameters: { type: "object", properties: { kind: { type: "string" }, match: { type: "object" }, orderBy: { type: "array", items: { type: "object", properties: { path: { type: "string" }, dir: { type: "string", enum: ["asc", "desc"] } }, required: ["path"] } }, limit: { type: "integer" } }, required: ["kind"] } } },
