...
| `ts/otlp.ts` | threads as OTLP traces, attempts as spans: a CLIENT that pushes, the way git-serve is a client that listens. Deterministic content-derived ids (re-export dedupes in the collector), the first IN-EXPORT parent nests and every other parent becomes a Link (a parentSpanId naming a span the collector never got is Jaeger's "not in the trace" warning + Incomplete badge; the follower backfills ancestry for the same reason), services resolve run → agent through `agent_run` RECORDS (the principal string carries no name), taint travels as label attributes, unsettled work says `radia.open` rather than inventing an end. Runnable form: `radia otlp --to <collector> (--thread <id> \| --follow) [--trace-root <kind>]`; any OTLP/HTTP collector (Jaeger v2, Tempo, Alloy) accepts it on `/v1/traces` |
