...
**Saving works from both directions.** `save_content` (tool-worker) stores text the assistant
...
| `chat/execworker.ts` | exec-worker: `tool_call{run_code}` → permissionless subprocess → **tainted** `tool_result`, and stdout → an **artifact** on `save_as` or past 4 KB (holds a run token; never executes anything itself) |
...
| `chat/save.ts` | `save_content`: store assistant-written text as an artifact (media type from the extension; write-only) |
