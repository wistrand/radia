...
| `extensions/`                           | conventions built ON the substrate that more than one app wants and the runtime has no business knowing: `ts/workspace.ts` (multi-file trees), `sandbox` records to come. Imports the SDK, NEVER `src/` — that rule is what keeps the tier real. Two surfaces are NORMATIVE (`treeDigestOf`, `validatePath`) because they cross a trust boundary, so `conformance/` there is a contract for any implementation, not a regression net. Ships in the npm package; not covered by the frozen wire contract. See [extensions/README.md](extensions/README.md) |
...
adapters), `deno task extensions` (the extension contract), `deno task chat-test` (the chat example, no API key), `deno task bench` (hotspots + scaling), `deno task demo`
