...
| `src/surfaces/cli.ts`                   | the CLI verbs (health/stats/doctor/erasures/flows/integrity/kinds/put/query/take/ack/watch/otlp/workspaces/workspace-git/git-serve/…), public `/v0` only. `git-serve` is a CLIENT that happens to listen: it binds its own port, so serving git needs no runtime change and no wire-contract entry |
