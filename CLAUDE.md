...
- [agent_docs/plan-console-auth.md](agent_docs/plan-console-auth.md): PROPOSAL, nothing built: ending the console's paste-a-token treadmill by porting the definition/run exchange to the page (the mechanism every other client already has), a CLI fragment handoff, a labeled operator button for open mode, and run-token renewal. Carries the security accounting and the rejected alternatives (cookies, OIDC-now).
