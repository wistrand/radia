...
- [agent_docs/plan-scaling.md](agent_docs/plan-scaling.md): how many users a space serves, and what raises the number. Three ceilings that bind in order (the per-user 9-process fleet, worker concurrency, watch fan-out), each with the measurement behind it. The RUNTIME half is BUILT (kind-aware wakeup, `idx_events_xid_seq`, the shared log read: a write costs 2 queries however many streams are parked); what remains is app/SDK, chiefly the shared-session shape and its consequence, SCOPE DELEGATION as a DELEGATED RUN (item 3a: a worker mints a run scoped to `grants(worker) ∩ grants(caller)` and holds two credentials — its own for its own capability, since exec writes `check` where the session may only read it, and the delegated one for the caller's data; minting rather than annotating each call is where AWS session policies, RFC 8693 and Kerberos all landed, and it keeps `effectivePermissions` a flat list; distinct from the chain-intersection gate design-auth.md defers). Carries the concurrency rule (bound at whatever outside thing judges the work: a vendor's rate limit, a wall-clock deadline; be generous where only we pay) and the within-kind routing verdict (measured at ~9µs/stream, deferred). Read before tuning a worker's concurrency or proposing a fan-out change.
- [agent_docs/plan-delegation.md](agent_docs/plan-delegation.md): PLANNED, the build sequence for delegated runs (plan-scaling.md item 3a). A worker mints a run scoped to `grants(worker) INTERSECT grants(caller)` and holds two credentials. Three things decide whether it is a guarantee or decoration: the caller is resolved through the RUN behind `created_by` (never a body field, never the leased record's author, since in the chat that names another worker); a plain intersection cannot narrow the worker, so the authority it holds only for callers is a grant to a `delegable:<worker>` principal no credential can ever resolve to; and `authorize`'s two early returns (privileged, the supervisor carve-out) skip any attenuation, so the MINT refuses those agents rather than trusting the check. Read before touching `authorize`, `grantSubject`, or `EXEC_GRANTS`.
