...
covered by any test), `deno task conformance` (both adapters, and what any `src/` change takes), `deno task extensions` (the extension contract), `deno task workspace-git` (a workspace as a git repo), `deno task chat-test` (the chat example, no API key), `deno task analysis-test` (the staged pipeline), `deno task bench` (hotspots + scaling), `deno task demo`
...
- [agent_docs/research-substrate-lessons.md](agent_docs/research-substrate-lessons.md): what building the chat and the analysis pipeline taught us about the substrate, with a claim ledger and six sized proposals. The findings that generalise: `indexedPaths` is the real API surface (both apps' core decision was which fields to declare), content addressing carries three unrelated features the docs do not lead with, and EVERY non-trivial bug across both apps was one hazard — a projection over an append-only log read as state. The gaps both apps hit independently: no scoped ops READ tier (`observe` is all-or-nothing and the self-scope tier needs `createdBy: "self"` on every grant, which fails when a WORKER authors your results), and no CORS, so every browser app proxies. Read before adding an app-level convention, or before deciding a substrate gap is one app's problem.
- [agent_docs/plan-analysis-workspace-agents.md](agent_docs/plan-analysis-workspace-agents.md): PLANNED: the analysis pipeline's stages as workspace agents (research-substrate-lessons.md action 5, full shape). The memo's `codeDigest` becomes the pinned treeDigest, `stage_code` is deleted (bindings discover, pins enforce, `digest_mismatch` catches disagreement), and promotion pins BOTH sides (`take` on the request, `put` on the result, so a result cannot lie about which code produced it). Its one substrate prerequisite is BUILT (host-side input materialisation, `Binding.inputs`: the host fetches declared inputs under the AGENT's authority into `input/` in the run's cwd, since broker frames carry no bytes and the jail has no net). Ends with the pipeline SHAPE as a `stage_def` registry replacing the `STAGES` constant, which is what lets a chat-authored workspace become a NEW stage (def + promote + bind) rather than only a new implementation. Read before touching `examples/analysis/` deployment or `extensions/ts/host.ts` inputs.
