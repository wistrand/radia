...
- [agent_docs/plan-ops-tiers.md](agent_docs/plan-ops-tiers.md): splitting the one operator bit. The seven bundled powers are named in design-auth.md ("The operator bit"); the decided shape is ops powers as reserved `ops_grant` records, assigned by config operators, fail-closed, with the escalation root (grant/identity writes) and the coordination bypass never below the full tier. Designed, not built. Read before touching `isPrivileged`, the ops gate, or the supervisor's authority.
