...
- [agent_docs/plan-chat-web-ui.md](agent_docs/plan-chat-web-ui.md): PLANNED, two stages in a deliberate order: a `ChatUI` PORT plus a DOM implementation (the fleet stays Deno processes), then the fleet in the tab. The measurement that decides it: the chat's logic is Deno-free but ANSI-BOUND — `client/turn.ts` calls eleven `terminal.ts` functions at 44 sites, and `markdown.ts` emits ANSI — so the port is getting the renderer out of the protocol, one refactor both stages need. Stage 1 ships alone and validates streaming, tool calls and progress in a browser before anything moves; stage 2 makes each worker a Web Worker (the provider key lands there, not in the page), reuses the jail for exec, and carries the honest costs: no OS isolation, key only as safe as the origin, spend caps required, OpenRouter CORS unverified. Read before touching `examples/chat/client/` rendering or proposing a chat port.
