...
| 5 | Pin stage code with promotion instead of self-report | turns the memo's foundation from a claim into an enforced fact, and would be the first worked composition of promotion with something other than an exec runner. **PLANNED in full** (stages as workspace agents): [plan-analysis-workspace-agents.md](plan-analysis-workspace-agents.md) | MEDIUM | `examples/analysis/`, `extensions/ts/promotion.ts` |
