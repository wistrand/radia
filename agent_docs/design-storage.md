...
| Lineage      | `parent_ids` on the record is the source of truth (walked upward, one batched fetch per depth level); `record_edges (parent_id, child_id)` is a derived REVERSE index for `childrenOf`, written in the same transaction as the record and backfilled from `parent_ids` for databases from an older build |
