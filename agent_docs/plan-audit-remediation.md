...
> **PACKAGE AC (2026-09-19) IS OPEN**: six problems the cluster benchmark found or left behind (two fixed since),
...
| AC | Cluster benchmark: an unexecuted TLS path, 500 for an outage, and two smaller gaps | P2/P3 | **OPEN** (opened 2026-09-19; AC1 and AC4 fixed) |
...
|---|---------|----------|----------------------------|
| 1 | ~~A black-holed database hangs every instance~~ FIXED 2026-09-19 | P2 | Nothing timed out a Postgres query and no socket set TCP keepalive, so a host that vanished without resetting its connections left every pooled connection waiting until the kernel gave up, and every request waiting on the pool (the benchmark's `partition` recovered only because its proxy held the bytes). Now every step of `withConn` has a deadline (`operationTimeoutMs`, default 30s): waiting for a slot, connecting, and the operation. One that misses it fails `database_unavailable` (503) and its client is REPLACED, never reused, since the abandoned driver query would later release its connection lock into another session. Guard: two black-hole cases in `test/pgreconnect.test.ts` (both hung on the old code; ~1s against an 800ms deadline now) |
...
| 3 | A database outage reaches clients as `500 internal` (a TIMEOUT now answers 503, AC1) | P3 | A lost connection surfaces from the driver as an unclassified error, so the catch-all answers 500 where the request was fine and a retry would succeed (`blob_store_unavailable` already answers 503 for the same shape). An SDK or load balancer treats the two differently. Candidate: classify driver connection errors as `database_unavailable`, 503, in `statusFor` (`src/server/problem.ts`) |
| 4 | ~~An empty events page costs a second query on Postgres~~ FIXED 2026-09-19 | P3 | `getEvents` asked `#aheadOf` (`pg_snapshot_xmax`) whenever a page was empty, and the notifier's cross-instance poll reads one event every `CHANGE_POLL_MS` (250ms) per instance while a stream is parked, which on an idle space is usually empty. Now one statement: a CTE computes `xmax` and a `left join lateral` over the page always yields a row, so an empty page is one row of nulls carrying `next`. Measured on PGlite: 100 empty pages, 100 statements (was 200). Guard: the ahead-cursor case in `test/conformance/suites/events.ts`, on every adapter |
