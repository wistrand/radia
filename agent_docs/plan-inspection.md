...
- **"Who would receive this record?"** becomes a query. The reusable machinery is `compile` +
  ...
  deferred inverted-index work. Say so plainly, so nobody expects it to hold if interests ever
  grow per-record.
...
- **Shape-aware collapsing.** A burst of 400 `llm_chunk` events is one fact, not 400 rows, so
  collapse runs of same-kind/same-parent events into a count plus a sparkline. The `excludeKinds`
  hack in `getGraph` is the current workaround; aggregation is the answer.
...
- **Time as an axis.** Lay the DAG left-to-right by creation time and overlay envelope spans
  ...
  distributed tracing's most legible artifact, and it derives entirely from the event log and
  envelope.
...
- **Overlay the other two stories, though they are not equal work.** The graph shows `parent_ids`
  only; `taint` and `delegation_context` are the other two things a record carries.
  ...
  - *Taint overlay: needs substrate work first.* Taint is one bit, lives in `runtimeMeta` rather than
    ...
    provenance is recorded, which is the same missing primitive the containment application needs,
    so do it once for both.
...
- **High-level verbs for the compositions LLMs get wrong.** `thread(recordId)` gives the full,
...
- **Evidence-linked narrative.** Any LLM-generated answer about the space should carry the record ids
  it rests on, rendered as console links. That keeps freeform narration honest. It is the
  calibration principle applied to the inspector itself: a story about the space is a claim; the
  linked records are the measurement.
