...
**Ephemeral liveness is answered three different ways, and capability advertisements never
...
crashed sole fleet's tools stay advertised until someone cleans up by hand. The other two
answers: `interest` entries are live while their RUN is (`liveInterests`, `src/core/gc.ts`), and
`progress` treats ABSENCE as the signal (`extensions/ts/progress.ts`, the chat's stall
diagnosis). The MUD's `presence` is not an instance: latest-wins location state, no TTL. The
chat's shape is already the generic convention: newest record per id inside a TTL, keyed per
refresh WINDOW so a long-lived heartbeat stays a handful of records. Heartbeat timers stay
client-side; the space never fires one. Action 7, planned in [plan-presence.md](plan-presence.md).
...
| 7 | An ephemeral presence convention in `extensions/` | liveness is answered three different ways (`chat_fleet` TTL beats, run-scoped `interest`, `progress` recency), and capability advertisements never expire, so a crashed sole fleet's tools stay advertised forever. The chat's shape is the prototype: newest record per id inside a TTL, keyed per refresh window. **BUILT 2026-08-30: [plan-presence.md](plan-presence.md)** | SMALL | `extensions/ts/presence.ts`, `examples/chat/client/fleet.ts` converted |
| 8 | `collapseByTool` fails closed on a conflict | the flag plus one console notice was the whole defence, and newest-wins let a `capability: put` holder substitute a definition. Was blocked on 7, since without liveness a stale advertisement from a crashed process would take a working tool offline indefinitely. **BUILT 2026-08-30 as phase 4 of [plan-presence.md](plan-presence.md)**: a contested name is withheld and reported, `onConflict: "newest"` opts out | SMALL, AFTER 7 | `extensions/ts/capability.ts`, `examples/chat/client/turn.ts` |
