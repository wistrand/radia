...
Deno.test("tasks: every `deno task` a workflow or script runs is defined in deno.json", async () => {
  ...
  // deno.json itself is a caller: a composite task (`test` runs `deno task check && …`) names
  // other tasks, and renaming one of those fails at run time with "Task not found" and nowhere
  // else. Same failure this file was written for, one level in.
  const callers = [
    ...await filesIn(".github/workflows", ".yml"),
    ...await filesIn("scripts", ".sh"),
    { name: "deno.json", text: JSON.stringify(config.tasks) },
  ];
