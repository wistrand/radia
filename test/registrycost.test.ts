...
// answer confidently about it. The safe inputs (`queryAll`, `readRegistry`, `query_all`) page to
...
const EXHAUSTIVE = /\b(queryAll|query_all|readRegistry|registry|liveInterests|readBindings|readRegistryOf)\s*(<[^>]*>)?\s*\(/;
...
Deno.test("[registry-cost] a latest-wins projection is never fed a bounded read", async () => {
  ...
  assertEquals(
    ...
    "a latest-wins projection over a bounded read answers confidently about a PREFIX. " +
      "Use queryAll/readRegistry, which page to exhaustion and report an incomplete view.",
  );
});

Deno.test("[registry-cost] no readRegistry caller states a page direction", async () => {
  // `readRegistry` builds the whole `Page` and hands it over, so a caller passes it through and
  ...
  for (const root of ROOTS) {
    for (const file of await tsFiles(root)) {
      ...
      const lines = text.split("\n");
      for (const [i, line] of lines.entries()) {
        if (!/\breadRegistry\s*(<[^>]*>)?\s*\(/.test(line)) continue;
  ...
  assertEquals(
    violations,
    [],
    "readRegistry builds the Page; a caller passes it through rather than restating the direction",
  );
});
...
Deno.test("[registry-cost] every escape from the Population brand is accounted for", async () => {
  ...
  const allowed = new Map([
    ["sdk/ts/registry.ts", 1], // readRegistry, where paging to exhaustion earns it
